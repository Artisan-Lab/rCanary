...
// This module can be used in RLC
// and is also available for heap cost analysis for new Rust RFC.

// The binary is automated emitted by 'install_rlc.sh' when building all components in rlc
...
// Users can also use rlc_phase_llvm to select one ll-file only if they pass the path to this tool.
...
// The Global CLI Argument for 'main' and the argument is actually the '.ll' file in rlc
...
// and the default dir in unix-like os is '/tmp/rlc-llvm-ir/*.ll' that we use rlc to construct.
int main(int argc, char **argv) {

    // If no path passing to rlc_phase_llvm, through an error and exit.
    if (argc == 0) {
        errs() << "Failed due to lack of input LLVM-IR file for rlc_phase_llvm\n";
        exit(1);
    }
